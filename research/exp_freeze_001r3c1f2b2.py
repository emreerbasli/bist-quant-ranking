"""Build and verify the immutable V10 forward-runtime manifest; never activates a runner."""
...
OUT = RESEARCH / "forward_infrastructure" / "FORWARD_INFRASTRUCTURE_MANIFEST_V10.json"
BOUND_PATHS = (
    ...
    RESEARCH / "exp_freeze_001r3c1f2b2.py",
...
def build_document() -> dict:
    contract = json.loads(fw.CONTRACT.read_text(encoding="utf-8"))
    return {
        "manifest_schema_version": "V10",
        "experiment_id": "EXP-FREEZE-001R3C1F2B2",
        ...
        "v9_manifest_sha256": fw.sha(RESEARCH / "forward_infrastructure" / "FORWARD_INFRASTRUCTURE_MANIFEST_V9.json"),
...
def write_once(output: Path, document: dict) -> str:
    ...
    if output.exists():
        if not sidecar.exists() or sidecar.read_text(encoding="utf-8").strip() != fw.sha(output):
            raise fw.ForwardIntegrityError("V10_MANIFEST_HASH_MISMATCH")
        existing = json.loads(output.read_text(encoding="utf-8"))
        if identity(existing) != identity(document):
            raise fw.ForwardIntegrityError("V10_MANIFEST_CONTENT_MISMATCH")
        return "V10_READ_ONLY_REUSE"
    if sidecar.exists():
        raise fw.ForwardIntegrityError("V10_MANIFEST_SIDECAR_WITHOUT_MANIFEST")
    fw.atomic_json(output, document)
    sidecar.write_text(fw.sha(output) + "\n", encoding="utf-8")
    return "V10_GENERATED"
...
def verify(output: Path = OUT) -> int:
    ...
    if document.get("status") != "PENDING_INDEPENDENT_AUDIT" or document.get("clean_forward_start") != "NOT_STARTED":
        raise fw.ForwardIntegrityError("V10_MANIFEST_STATUS_MISMATCH")
    if document.get("candidates") != candidates():
        raise fw.ForwardIntegrityError("V10_CANDIDATE_HASH_MISMATCH")
    for rel, expected in document.get("hashes", {}).items():
        if fw.sha(ROOT / rel) != expected:
            raise fw.ForwardIntegrityError("V10_BOUND_FILE_HASH_MISMATCH")
