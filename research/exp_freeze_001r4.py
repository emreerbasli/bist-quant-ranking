"""Build and verify the immutable V11 ADV20-parity forward manifest; never activates a runner."""
...
OUT = RESEARCH / "forward_infrastructure" / "FORWARD_INFRASTRUCTURE_MANIFEST_V11.json"
BOUND_PATHS = (
...
def build_document() -> dict:
    contract = json.loads(fw.CONTRACT.read_text(encoding="utf-8"))
    return {
        "manifest_schema_version": "V11",
        "experiment_id": "EXP-FREEZE-001R4",
        ...
        "v10_manifest_sha256": fw.sha(RESEARCH / "forward_infrastructure" / "FORWARD_INFRASTRUCTURE_MANIFEST_V10.json"),
...
def write_once(output: Path, document: dict) -> str:
    ...
    if output.exists():
        if not sidecar.exists() or sidecar.read_text(encoding="utf-8").strip() != fw.sha(output):
            raise fw.ForwardIntegrityError("V11_MANIFEST_HASH_MISMATCH")
        existing = json.loads(output.read_text(encoding="utf-8"))
        if identity(existing) != identity(document):
            raise fw.ForwardIntegrityError("V11_MANIFEST_CONTENT_MISMATCH")
        return "V11_READ_ONLY_REUSE"
    if sidecar.exists():
        raise fw.ForwardIntegrityError("V11_MANIFEST_SIDECAR_WITHOUT_MANIFEST")
    fw.atomic_json(output, document)
    sidecar.write_text(fw.sha(output) + "\n", encoding="utf-8")
    return "V11_GENERATED"
...
def verify(output: Path = OUT) -> int:
    ...
    if document.get("status") != "PENDING_INDEPENDENT_AUDIT" or document.get("clean_forward_start") != "NOT_STARTED":
        raise fw.ForwardIntegrityError("V11_MANIFEST_STATUS_MISMATCH")
    if document.get("candidates") != candidates():
        raise fw.ForwardIntegrityError("V11_CANDIDATE_HASH_MISMATCH")
    for rel, expected in document.get("hashes", {}).items():
        if fw.sha(ROOT / rel) != expected:
            raise fw.ForwardIntegrityError("V11_BOUND_FILE_HASH_MISMATCH")
