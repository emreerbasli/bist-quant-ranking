...
===================================
V4 PRODUCTION: VERİ YÜKLEYİCİ VE YARDIMCI HESAPLAMA MOTORU (TAM İZOLASYON)
-------------------------------------------------------------------------
Bu modül, dondurulmuş V4 9-faktörlü modelin Point-in-Time (PIT) temel verilerini,
fiyat serilerini, makro faiz ve enflasyon göstergelerini eski sürümlerden (V3)
tamamen bağımsız olarak diskten yükleyen ve hesaplayan kurumsal veri motorudur.
"""
...
logger = logging.getLogger("V4DataLoader")
...
def yukle_veriler():
    ...
    logger.info(f"V4 Veriler yüklendi: {len(fiyat_dict)} hisse | PIT: {len(pit_bellek)} | Cache: {len(cache_bellek)}")
...
def check_market_date_alignment(
    ...
    """Tüm hisselerin ve XU100'ün aynı PIT işlem gününü paylaştığını doğrular."""
    if seri_xu100.empty:
